 ...
 * `{resource, config, remote}` plus the two registration-time-only members (`routes`/`resolve`)
 * that `onStart`/`onStop` deliberately never get, because composition has already finished by the
...
export function buildSetupContext(
  ...
  return {
    ...buildRuntimeContext(def, resources, remoteCaller),
