...
import { getConfigOverride, hasConfigOverride } from './config-overrides.ts'
 ...
 *
 * `config.get` checks the Config Plane hot-refresh overlay (`config-overrides.ts`) FIRST — the
 * only way a `remote` instance subscribed via `announceRemoteInstance` ever sees a live push
 * reflected here — falling back to the manifest's own declared default when no override was ever
 * set (an `embedded` app, or one that never declared non-secret `config`, always falls back —
 * that module stays untouched for it). `config.has` only checks `def.config`'s own declared keys,
 * never the override overlay directly — safe because an override can only ever exist for a key
 * already declared there (`announceRemoteInstance` only ever subscribes declared, non-secret
 * keys), so the two can never disagree in practice. Resolving a HOST-level static override
 * (`apps.<n>.config`, `@zanix/core`'s own composition step) is separate and still not wired in;
 * once it normalizes into `def.config` itself, nothing here needs to change.
 *
...
export function buildRuntimeContext(
  ...
  return {
    ...
    config: {
      get: (key: string) =>
        hasConfigOverride(def.name, key)
          ? getConfigOverride(def.name, key)
          : def.config[key]?.default ?? undefined,
